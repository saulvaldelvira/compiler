        ...
        let kind = match &expr.kind {
            EK::Unary { op, expr } => {
                match op.val {
                    ...
                    UnaryExprOp::Negation => HExprKind::Unary { op: UnaryOp::Neg, expr: self.lower_expression(expr) },
                    UnaryExprOp::Not => HExprKind::Unary { op: UnaryOp::Not, expr: self.lower_expression(expr) },
            ...
            EK::Ternary { cond, if_true, if_false } => {
                ...
                HExprKind::Ternary { cond, if_true, if_false }
            },
            EK::Path(spanned) => {
                HExprKind::Variable(self.lower_path(spanned))
            },
            EK::Literal(spanned) => {
            ...
            EK::StructAccess { st, field } => {
                ...
                HExprKind::StructAccess { st, field: ident(field) }
            },
        };
