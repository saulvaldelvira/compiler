...
pub trait Visitor<'hir> {
    ...
    fn visit_arithmetic(&mut self, _base: &'hir Expression<'hir>, left: &'hir Expression<'hir>, op: &ArithmeticOp, right: &'hir Expression<'hir>) -> Self::Result {
    ...
    fn visit_field(&mut self, def: &'hir Definition<'hir>, param: &'hir Field<'hir>) -> Self::Result {
    ...
    fn visit_block(&mut self, _base: &'hir Statement<'hir>, block: &'hir [Statement<'hir>]) -> Self::Result {
    ...
    fn visit_return(&mut self, _base: &'hir Statement<'hir>, ret: Option<&'hir Expression<'hir>>) -> Self::Result {
        walk_return(self, ret)
    }

    fn visit_print(&mut self, _base: &'hir Statement<'hir>, expr: &'hir Expression<'hir>) -> Self::Result {
        walk_print(self, expr)
    }

    fn visit_read(&mut self, _base: &'hir Statement<'hir>, expr: &'hir Expression<'hir>) -> Self::Result {
        walk_read(self, expr)
    }
...
pub fn walk_field<'hir, V>(v: &mut V, def: &'hir Definition<'hir>, f: &'hir Field<'hir>) -> V::Result
where
    V: Visitor<'hir> + ?Sized
{
...
pub fn walk_logical<'hir, V>(
    v: &mut V, left: &'hir Expression<'hir>,
    _op: &LogicalOp,
    right: &'hir Expression<'hir>
) -> V::Result
where
...
pub fn walk_return<'hir, V>(
    v: &mut V,
    expr: Option<&'hir Expression<'hir>>,
) -> V::Result
where
    V: Visitor<'hir> + ?Sized
{
    ...
    V: Visitor<'hir> + ?Sized
{
    ...
    match &expr.kind {
        ...
        ExpressionKind::Call { callee, args } => { v.visit_call(expr, callee, args); }
        ExpressionKind::ArrayAccess { arr, index } => { v.visit_array_access(expr, arr, index); }
        ExpressionKind::StructAccess { st, field } => { v.visit_struct_access(expr, st, *field);}
    }
    ...
    V: Visitor<'hir> + ?Sized
{
    ...
    match &stmt.kind {
        ...
        StatementKind::For { init, cond, inc, body } => v.visit_for(stmt, *init, *cond, *inc, body),
